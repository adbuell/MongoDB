...
class Config {
    ...
    async load() {
        ...
        s3.templates = Object.assign({ platformBaseDir: '<%- bin %>', vanillaBaseDir: '<%- bin %>', versionedPlatformTarball: "<%- name %><%- channel === 'stable' ? '' : '/channels/' + channel %>/<%- bin %>-v<%- version %>/<%- bin %>-v<%- version %>-<%- platform %>-<%- arch %>", platformTarball: "<%- name %><%- channel === 'stable' ? '' : '/channels/' + channel %>/<%- bin %>-<%- platform %>-<%- arch %>", versionedVanillaTarball: "<%- name %><%- channel === 'stable' ? '' : '/channels/' + channel %>/<%- bin %>-v<%- version %>/<%- bin %>-v<%- version %>", vanillaTarball: "<%- name %><%- channel === 'stable' ? '' : '/channels/' + channel %>/<%- bin %>", platformManifest: "<%- name %><%- channel === 'stable' ? '' : '/channels/' + channel %>/<%- platform %>-<%- arch %>", vanillaManifest: "<%- name %><%- channel === 'stable' ? '' : '/channels/' + channel %>/version" }, s3.templates);
        await Promise.all([
